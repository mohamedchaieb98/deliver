...
def update_client(
    client_id: UUID, client_in: ClientUpdate, db: Session = Depends(get_db)
):
    """Mettre à jour les infos d'un client"""
    # exclude_unset=True pour ne mettre à jour que les champs envoyés
    client = crud_client.update_client(
        db, client_id=client_id, update_data=client_in.dict(exclude_unset=True)
    )
    if not client:
        raise HTTPException(status_code=404, detail="Client non trouvé")
    return client
