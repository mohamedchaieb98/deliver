...
from app.schemas.reseller import ResellerCreate, ResellerUpdate, ResellerResponse
...
def update_reseller(
    ...
    reseller = crud_reseller.update_reseller(db, reseller_id=reseller_id, update_data=reseller_in.dict(exclude_unset=True))
    if not reseller:
        raise HTTPException(status_code=404, detail="Reseller non trouvé")
    return reseller
