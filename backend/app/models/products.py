...
class Supplier(Base):
    ...
    supplier_orders = relationship(
        ...
        foreign_keys="SupplierOrder.supplier_id",
    )
    supplier_products = relationship(
        "SupplierProduct",
        back_populates="supplier",
        foreign_keys="SupplierProduct.supplier_id",
    )
...
class SupplierOrder(Base):
    ...
    order_items = relationship(
        "OrderItem", back_populates="supplier_order", cascade="all, delete-orphan"
    )
...
class OrderItem(Base):
    ...
    supplier_order_id = Column(
        String(36), ForeignKey("supplier_orders.id"), nullable=False, index=True
    )
    product_id = Column(
        String(36), ForeignKey("products.id"), nullable=False, index=True
    )
